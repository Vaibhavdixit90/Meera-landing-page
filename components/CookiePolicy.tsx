...
interface CookiePolicyAttributes {
  Tittle: string;
  Description: string;
}

interface CookiePolicyResponse {
  data: {
    id: number;
    attributes: CookiePolicyAttributes;
  }[];
  meta: {
    pagination: {
      page: number;
...
const CookiePolicy: React.FC = () => {
  const [cookieData, setCookieData] = useState<CookiePolicyAttributes | null>(
    null,
  );
    ...
    const fetchCookiePolicy = async () => {
      try {
        const response = await fetch(
          "https://cms.flowautomate.io/api/pages?filters[slug][$eq]=cookie-policy",
        );
        const data: CookiePolicyResponse = await response.json();
        if (data.data.length > 0) {
          setCookieData(data.data[0].attributes);
        } else {
          setError("Cookie policy not found");
        }
      } catch (err) {
        setError("Failed to fetch cookie policy");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="relative mx-auto mt-24 max-w-7xl px-4 md:mt-28 md:px-8">
      ...
      <div
        ...
        dangerouslySetInnerHTML={{ __html: cookieData?.Description || "" }}
      />
    </div>
  );
};
